...
object DataWarehouse extends  App{
    ...
    if (lastfileRead == null ){
      println("I am inside lastfileRead == null")
        val df1 = spark.read.format("csv").option("header","true")
          ...
          .load("s3a://twitter-khyber/test-code/*/*.csv")
      ...
      */
      ...
      val dftable = spark.read.jdbc("jdbc:postgresql://database-1.c6vrj3gbkajn.us-east-1.rds.amazonaws.com:5432/postgres", pgTable,pgConnectionProperties).cache()
      ...
      val temp_df = dftable.join(refined_df,refined_df("emp_idu")===dftable("emp_id"), "leftouter" )
      println("temp_df")
      temp_df.show()
      val temp_df1 =
...
//        .save()
      ...
      val jdbcUrl = "jdbc:postgresql://database-1.c6vrj3gbkajn.us-east-1.rds.amazonaws.com:5432/postgres"
        ...
        if (cnt != lastcount){
          ...
          dfinsert_renamed.join(dftable,dftable("emp_id")===dfinsert_renamed("emp_idi"), "leftouter")
          ...
          val temp_df = dftable1.join(refined_df,refined_df("emp_idu")===dftable1("emp_id"), "leftouter" )
          val temp_df1 =
            temp_df.withColumn("ofc_location",when(col("emp_idu").isNotNull, col("ofc_locationu")).otherwise(col("ofc_location")))
              .withColumn("updated_at",when(col("emp_idu").isNotNull, col("updated_atu")).otherwise(col("updated_at")))
          temp_df1.show()
          temp_df1.select("emp_id","ofc_location","updated_at")
            .write.format("jdbc")
...
if (lastfileRead == lastfileAdded){
  ...
  if (cnt!=lastcount) {
    ...
    /*
     ...
     filter op is insert as dfinsert
     dftable is maintable as dataframe
     chnage columnn name of dfinsert
     join dfinsert and dftable on primary key
     whereever join is giving null filter those data and columns and insert them
     ...
     flter op as d as dfdelete and recent one
     dftable is maintable as dataframe
     chnage columnn name of dfdelete
     join dfdelete and dftable on primary key
     if join successful and dletetedtimestamp > rowtimestamp
     filetr throse above keys and dlete data
    ...
    if (!dfdelete.take(1).isEmpty) {
      ...
      val dftable2 = spark.read.jdbc("jdbc:postgresql://database-1.c6vrj3gbkajn.us-east-1.rds.amazonaws.com:5432/postgres", "public.testcode", pgConnectionProperties).cache()
      val dfdelete_renamed = dfdelete.select("emp_id","ofc_location","updated_at").toDF("emp_idd","ofc_locationd","updated_atd")
      dftable2.join(dfdelete_renamed,dftable2("emp_id") === dfdelete_renamed("emp_idd"), "leftouter")
        .filter(col("emp_idd").isNull)
        .select("emp_id","ofc_location","updated_at")
        .write.format("jdbc")
        .mode("overwrite")
        .option("url", jdbcUrl)
        .option("dbtable", "public.testCode2")
        .option("user", "postgres")
        .option("password", "9473249664")
        .save()
