...
import org.apache.spark.sql.functions.{trim, length, when, rtrim}

///home/prasoon/Downloads/dsw1/dsw/component
object ssw1validate extends App {
  ...
  val quoteDF = spark.read.parquet("/home/prasoon/Downloads/dsw3/dsw/quote").limit(300000)
  val componentDF = spark.read.parquet("/home/prasoon/Downloads/dsw3/dsw/component").limit(800000)
  ...
  val headerDF = spark.read.parquet("/home/prasoon/Downloads/dsw3/dsw/header").limit(400000)
  ...
  val df =newquoteDF.join(newcomponentDF,newquoteDF("COUNTRY_CODE_QUOTE")===newcomponentDF("COUNTRY_CODE_COMPONENT") &&  newquoteDF("QUOTE_ID_QUOTE")===newcomponentDF("QUOTE_ID_COMPONENT"))
    .join(newheaderDF,newquoteDF("COUNTRY_CODE_QUOTE")===newheaderDF("COUNTRY_CODE_HEADER") &&  newquoteDF("QUOTE_ID_QUOTE")===newheaderDF("QUOTEID_HEADER"), "leftouter").limit(400000)
    .select("QUOTE_ID_QUOTE")
...
//      && (!col("UPGRADE_FLAG_COMPONENT").isin('C','3'))
...
//    .withColumn("TRANS_ID_PLUS",substring(concat(substring(col("COUNTRY_CODE_QUOTE"), 1, 2),rtrim(col("QUOTE_ID_QUOTE")),rtrim(col("COMPONENT_ID_COMPONENT")),rtrim(substring(col("BRAND_CODE_COMPONENT"), 1, 2)),
...
//    .withColumn("LIST_PRICE",when(col("LIST_PRICE_LOCAL_COMPONENT")===999999999,0).otherwise(col("LIST_PRICE")))
...
//    .select("APPROVALCODE", "TRANSACTIONSTATUS", "CURRENCYCODE", "CONVERSIONCODE", "COSTCONVERSRATE",
...
//      "DISCOUNT", "BID_REVENUE", "BMC", "TMC", "BMC_GP", "TMC_GP", "MES_FL", "BM_RECORD_FL", "BELOWDELEGCODE", "DELEG_FL"
//    )
// // ssw.show()
//  //
//  //  //println(ssw.select("CURRENCYCODE").distinct().show())
//  //  println(ssw.select("TRANS_ID_PLUS").distinct().count())
//  //println(ssw.filter(col("CURRENCYCODE").isin("EUR","INR","TWD","KRW","KW","THB","CHF","ZAR","SGD","HKD")).count())
//
//  val validatedf= spark.read.option("header", "true").csv("/home/prasoon/Downloads/IW_SSW_Data_test.csv")
//  val  validatedf1 = validatedf.withColumn("TRANS_ID", rtrim(col("TRANS_ID")))
//    .withColumn("TRANS_ID_PLUS", rtrim(col("TRANS_ID_PLUS")))
//  //  validatedf1.show()
//  //  println(validatedf.count())
//  //ssw.join(validatedf1 , Seq("TRANS_ID_PLUS"), "inner").select("TRANS_ID_PLUS").count()
//  //  validatedf1.filter(col("TRANS_ID").like("%AL4147333%")).show()
//  //  ssw.filter(col("TRANS_ID")=== "%JP4208831%").show()
//
//  validatedf1.filter(col("PRODID")==="694212F").show()
//  ssw.filter(col("PRODID")==="694212F").show()
//  //ssw.filter(col("QUANTITY")>1).show()
